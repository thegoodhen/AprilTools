...
def markers_to_3d_points(marker_matrix_pairs, marker_size):
    points=[]
    for marker in sorted(marker_matrix_pairs):
        marker_matrix=marker_matrix_pairs[marker]
        corners=get_corner_base_coordinates(marker_size)
        newCorners=transform_corners_with_matrix(corners,marker_matrix)
...
def frame_data_to_2D_points(marker_matrix_pairs,frame_list,frame_number):
    '''

    :param marker_matrix_pairs: a dictionary, which assigns a transformation matrix to each marker ever observed in the video sequence
    :param frame_list: a list of frame data, containing the tracking information (i.e. 2D corner position) of the individual markers
    :param frame_number: the frame number (as index of frame_list) to extract the 2D points from
    :return:pointIDs: a list of indices, indexing an array of 3D points, as generated by markers_to_3D_points, determining
     the correspondences between the 3D points and returned 2D poinst;
     points2D: a numpy array of 2D points
    '''
    frame_data=frame_list[frame_number]
    ids=frame_data['ids'].ravel()
    corners=np.array(frame_data['corners'])

    #get rid of all the markers that we are not interested in, but that were still detected
    valid_marker_ids=marker_matrix_pairs.keys()#All markers that were detected in the video sequence
    marker_ids_to_point_ids_dict={}#a lookup dictionary to convert the marker indices to point indices
    for index, maker_id in enumerate(sorted(valid_marker_ids)):
        marker_ids_to_point_ids_dict[index]=index*4
    ...
    for i in range(len(ids)):
        ...
        points2Dtemp=points2Dtemp.ravel()
        points2Dtemp=points2Dtemp.reshape((4,2))
        points2D.extend(points2Dtemp)
        basePointID=marker_ids_to_point_ids_dict[ids[i]]
        for j in range(4):
            pointIDs.extend([basePointID+j])

    return [pointIDs,np.array(points2D)]
...
def prepare_data_for_bundle_adjustment3(frame_data, marker_matrix_pairs, marker_size, camera_matrix, distortion_coefficients):
    retPoints2D=[]
    pointIDs=[]
    viewportIDs=[]
    camVectorList=[]

    points3D=markers_to_3d_points(marker_matrix_pairs,marker_size)

    global framesCount
    for i in range(framesCount):
        pointIDsFrame, points2D=frame_data_to_2D_points(marker_matrix_pairs,frame_data,i)
        retval, rvecCam,tvecCam=cv2.solvePnP(points3D[pointIDsFrame],points2D,camera_matrix,distortion_coefficients)
        camVector=get_camera_vector(rvecCam,tvecCam,camera_matrix)
        #rvecCam[2]=rvecCam[2]+0.1
        #[points2DActualFrame, points2DProjectedFrame,Aframe,Bframe]=prepareBundleAdjustmentParamsForSingleFrame(points3D,points2D,rvecCam,tvecCam,camera_matrix,distortion_coefficients)
        retPoints2D.extend(points2D)
        ...
        pointIDs.extend(pointIDsFrame)
        camVectorList.append(camVector)
    return [np.array(pointIDs), np.array(viewportIDs), np.array(retPoints2D), np.array(points3D),np.array(camVectorList)]
...
frame_data_to_2D_points(markerMatrixPairs,frameDataList,5)
...
[point_indices,viewport_indices,points2D,points3D,camVector]=prepare_data_for_bundle_adjustment3(frameDataList,markerMatrixPairs,0.1,camera_matrix,distCoeffs)
